function coeff = CylShearDec4Dpar(BP, F)
%CYLSHEARDEC4D performs the 4D cylindrical shearlet decomposition using
%parallel computing.
...
for l = 1:level
    ...
    cl = zeros([szBP,numF], dataClass); % Initialize coefficient array
    ...
    parfor fi = 1:numF % Loop through all filters.
        ...
        cl(:,:,:,:,fi) = cylconv3d(exBP,Fl(:,:,:,fi),szBP(1:3));
    end % fi (filterIndex)
    coeff{l} = cl;
